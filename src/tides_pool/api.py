...
async def index() -> HTMLResponse:
    ...
    html = _re.sub(
        ...
        'src="/static/app.js?v=20260905contrib1"',
        html,
        count=1,
    )
    html = _re.sub(
        r'href="/static/style\.css(?:\?v=[^"]*)?"',
        'href="/static/style.css?v=20260905contrib1"',
        html,
        count=1,
    )
...
async def stats() -> PoolStats:
    ...
    hr_window = 600
    hr_window_1h = 3600
    # One query for 1h; split into 10m + 1h work for dual hashrate cards.
    recent_1h = await store.list_share_rows_since(hr_window_1h, limit=200_000)
    now_ts = datetime.now(timezone.utc).timestamp()
    cutoff_10m = now_ts - hr_window
    recent_work = 0
    recent_work_1h = 0
    recent_n_10m = 0
    for r in recent_1h:
    ...
    pool_hs_1h = estimate_hashrate_hs(recent_work_1h, hr_window_1h)
    ...
    # Rough 1 TH/s / day payout from 7d find rate (not a PPS promise).
    est_per_ths: float | None = None
    hs_ref = pool_hs_1h if pool_hs_1h > 0 else pool_hs
    if hs_ref > 0 and blocks_7d > 0 and reward_est > 0:
        miner_frac = max(0.0, (10_000 - int(settings.fee_bps or 0)) / 10_000.0)
        blocks_per_day = float(blocks_7d) / 7.0
        est_per_ths = blocks_per_day * float(reward_est) * miner_frac * (1e12 / hs_ref)
    ...
    return PoolStats(
        ...
        hashrate_shares=recent_n_10m,
        hashrate_hs_1h=pool_hs_1h,
        hashrate_window_1h_sec=hr_window_1h,
        network_hashrate_hs=net_hs,
        pool_network_share_pct=round(share_pct, 6),
        est_block_time_sec=est_block,
        est_sats_per_day_per_ths=est_per_ths,
    )
